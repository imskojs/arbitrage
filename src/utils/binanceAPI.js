...
// Get currency order book: symbol LTC/BTC, limit 5 10 20 50 100 500 1000
export function getOrderbook(symbol, limit = 100) {
...
export function buyLimit(symbol, quantity, rate) {
  ...
  return time().pipe(
    switchMap(({ serverTime }) => {
      const params = `symbol=${newSymbol}&side=BUY&type=LIMIT&quantity=${quantity}&price=${rate}&timeInForce=GTC&timestamp=${serverTime}`;
...
export function sellLimit(market, quantity, rate) {
  return time().pipe(
    switchMap(({ serverTime }) => {
      const params = `symbol=${market}&side=SELL&type=LIMIT&quantity=${quantity}&price=${rate}&timeInForce=GTC&timestamp=${serverTime}`;
      const signature = hmacSHA256(params, API_SECRET);
